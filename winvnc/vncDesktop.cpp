	...
	while (TRUE)
	{
		...
		if (msg.message == RFB_SCREEN_UPDATE)
		{
			...
			if ((rect.left < 0) || (rect.top < 0) ||
				...
				log.Print(50, VNCLOG("update:%d,%dx%d,%d\n"),
...
{
	HANDLE prop = GetProp(hwnd, (LPCTSTR) MKLONG(VNC_WINDOWPOS_ATOM, 0));
	if (prop != NULL)
	{
		if (IsWindowVisible(hwnd))
		{
			...
			if (GetWindowRect(hwnd, &dest))
			{
				...
				if ((source.x != dest.left) || (source.y != dest.top))
				{
					...
					SHORT y = dest.top;
					SetProp(hwnd,
						(LPCTSTR) MKLONG(VNC_WINDOWPOS_ATOM, 0),
						(HANDLE) MKLONG(x, y));
				...
				RemoveProp(hwnd, (LPCTSTR) MKLONG(VNC_WINDOWPOS_ATOM, 0));
		}
		else
		{
			RemoveProp(hwnd, (LPCTSTR) MKLONG(VNC_WINDOWPOS_ATOM, 0));
